...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstArg = x1 * x2 + y1 * y2;
  const secondArg = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(firstArg / secondArg);
}
...
function getFibonacciNumber(index) {
  ...
  if (index === 0) return fiboNumbers[index];
  if (index === 1) return fiboNumbers[index];

  for (let i = 2; i <= index; i += 1) {
    fiboNumbers.push(fiboNumbers[i - 2] + fiboNumbers[i - 1]);
  }

  return fiboNumbers.at(-1);
}
